...
void HuffmanTree::saveHuCode()
{
	ofstream hfmCode;
	// 打开文件
	hfmCode.open("hfmCode.txt", ios::in | ios::trunc);
	// 读写文件
	if (hfmCode.is_open()) {
		map<char, string>::iterator it;
		for (it = huCode.begin(); it != huCode.end(); it++)
			hfmCode << (*it).first << ":" << (*it).second << "\n";
		hfmCode.close();
	}
}
...
void HuffmanTree::treePrinting(int & postion, string str, ofstream & file)
{
	if (huTree[postion].weight == 0) {
		return;
	}
	str += "   ";
	treePrinting(huTree[postion].rchild, str, file);
	cout << str << huTree[postion].weight<<endl;
	file << str << huTree[postion].weight << endl;;
	treePrinting(huTree[postion].lchild, str, file);
}
